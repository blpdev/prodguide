...
');
					...
					?>
								<li style="text-indent:-18px;"><input type="checkbox" class="pdf_generator_checkselector" id="<?=$product->getId();?>" style="margin-right:3px;" data-product_id="<?=$product->getId();?>" data-product_name="<?=$product->getName();?>"><?=($this_one+1);?>. <a href="#" onclick="loadProductData(<?=$product->getId();?>); return false;"><?=$product->getName();?></a><!-- <span style="color:#999;">(<?=intval($product->getMostPopularCount());?>)</span>--><!-- <span><img class="add_to_pdf_icon" src="images/add_to_pdf_text.png" data-product_id="<?=$product->getId();?>" data-product_name="<?=$product->getName();?>"></span>--></li>
<?php
